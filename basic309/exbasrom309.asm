...
; basic309: adapted to run as a guest of bios/ instead of talking to the
; UART hardware directly. Console IO goes through the BIOS's SWI2 call
; convention (see bios/defines.d) instead of UT_DAT/UT_STA/UT_CMD/UT_CTL.
F_STDOUT    equ  $01           ; BIOS fileref: console output (aliased to the UART)
...
FOPEN_APPEND  equ $02
FOPEN_UPDATE  equ $03
ERR_BADDEV    equ $02          ; DOS error codes (bios/defines.d) that DOSERR translates
ERR_NOTFOUND  equ $05
ERR_NOSPACE   equ $06
...
NFILES        equ 4
FM_IN         equ 1            ; OPEN "I" / FOR INPUT
FM_OUT        equ 2            ; OPEN "O" / FOR OUTPUT
...
RTS_LOW   EQU  $95
; basic309: relocated workspace. bios/ owns $0000-$04FB (see bios/main.asm
; EndOfVars/USER_RAM); this whole block -- and every direct-page-style
; access to it below, all of which the assembler encodes automatically
; via SETDP, not by hand -- moves up to WORKBASE so it can't collide with
; BIOS's own reservation or DOS's. Runtime DP is set to match in RESVEC, below.
          ORG  WORKBASE
          SETDP WORKBASE/256
...
DEVNUM    RMB  1              0 = console; else the file a PRINT#/INPUT# is using
FT_MODE   RMB  NFILES+1       0 = closed, else FM_IN/FM_OUT/FM_APP/FM_RND
FT_REF    RMB  NFILES+1       DOS fileref of each open file
FT_COL    RMB  NFILES+1       each file's print column (for TAB and comma zones)
OPMODE    RMB  1              OPEN: the FM_ mode being requested
OPNUM     RMB  1              OPEN: the file number being requested
FI_REF    RMB  1              INPUT#/LINE INPUT#: DOS fileref being read
; Random-access files (OPEN "R"): per-file record length and current record number
; (2 bytes each, indexed by 2*n), and each file's record buffer (256 bytes,
; RBUF + 256*(n-1)) that FIELD variables point into.
RECLEN    RMB  2*(NFILES+1)
RECNO     RMB  2*(NFILES+1)
RBUF      RMB  256*NFILES
OPLEN     RMB  2              OPEN: requested record length (default 128)
RTMP      RMB  2              FIELD: offset of the next field in the record
FLDP      RMB  2              FIELD/LSET/RSET: address of the string descriptor
FLDW      RMB  1              FIELD width; LSET/RSET field width
FLDL      RMB  1              LSET/RSET: bytes of the source that fit
...
; THIS ROUTINE GETS A KEYSTROKE FROM THE CONSOLE IF A KEY IS DOWN, VIA
; THE BIOS'S SWI2 CALL CONVENTION (bios/defines.d). IT RETURNS ZERO TRUE
; IF THERE WAS NO KEY DOWN -- same contract as the original hardware-
; polling version this replaces.
KEYIN     PSHS B
          LDA  #B_GETC
          LDB  #F_STDIN
          SWI2
          BCS  NOCHAR         ; carry set: no character available
          ANDA #$7F
          PULS B
          RTS
NOCHAR    CLRA
...
; CONSOLE OUT, via the BIOS's SWI2 call convention.
PUTCHR    TST  DEVNUM         basic309: PRINT# etc. send output to a file
          LBNE PUTCHR_FILE
          PSHS A
...
; direct UT_DAT access this replaces) no separate wait loop is needed.
PUTCHR_RAW
          PSHS B
          TFR  A,E
          LDA  #B_PUTC
...
*                              
RESVEC
; basic309: set DP to match our relocated workspace's page (SETDP above)
; *before* anything -- including the very next instruction, which reads
; RSTFLG at its relocated address using the assembler's automatic
; direct-page encoding.
          LDA  #WORKBASE/256
          TFR  A,DP
...
BACDST    LDX  #PROGST+1      POINT X TO CLEAR THE BASIC WORKSPACE
LA077     CLR  ,--X           MOVE POINTER DOWN TWO-CLEAR BYTE
          LEAX 1,X            ADVANCE POINTER ONE
          CMPX #WORKBASE      AT THE BOTTOM OF OUR RELOCATED WORKSPACE?
          BNE  LA077          KEEP GOING IF NOT (was: BNE on X==0 --
                              ; bios/ owns everything below WORKBASE now,
                              ; so this must stop there instead)
          LDX  #PROGST        SET TO START OF PROGRAM SPACE
          CLR  ,X+            CLEAR 1ST BYTE OF BASIC PROGRAM
          STX  TXTTAB         BEGINNING OF BASIC PROGRAM
; basic309: TOPRAM is a fixed constant instead of an open-ended upward
; probe. The interpreter's own code now lives in RAM just above this
; (loaded at $C000); an unbounded probe expecting to naturally hit ROM
; would instead walk into and corrupt the interpreter's own live code
; before ever reaching bios/'s ROM at $F000.
          LDX  #TOPRAM_FIXED
...
; basic309: the original 4-byte copy into IRQVEC here is dropped -- that
; slot (and SW3VEC/SW2VEC/FRQVEC/SWIVEC/NMIVEC alongside it) is never
; read or jumped through anywhere else in this source; bios/'s own
; RAM_JTAB owns the real hardware vectors now.
          LDA  #$39
...
; basic309: no UART_INIT call here -- bios/'s own cold-start already
; programmed and initialized the UART before basic309 ever gets control.
...
* basic309: both counts are derived, never hand-edited: the primary count is
* the number of dictionary words (executable statements + the 19 fixed
* non-executable words ending at USING), the secondary count is the FUNC_TAB
* length. A stale count silently hides the LAST dictionary entries from the
* crunch routine (>,=,<,FN,USING stopped tokenizing once) -- see
* simulator/tests/test_basic309_token_audit.cpp.
COMVEC    FCB  TOK_USING-$7F  BASIC COMMANDS (RESERVED WORDS IN LAA66)
          FDB  LAA66          POINTS TO RESERVED WORDS 
          FDB  LAB67          POINTS TO JUMP TABLE FOR COMMANDS 
          FCB  NUM_SEC_FNS    SECONDARY COMMANDS (FUNCTIONS IN LAB1A)
...
LA35F     PSHS X,B,A          SAVE REGISTERS
          LDB  DEVNUM
          BNE  LA35F_FILE     basic309: printing to a file -- its own column
...
LA35F_FILE
          LDX  #FT_COL
          ABX
          LDB  ,X             THIS FILE'S PRINT COLUMN
          LDA  LPTCFW         SAME TAB FIELD WIDTH AS THE CONSOLE ...
          PSHS B
          LDB  #240           ... BUT THE LAST TAB ZONE IS FAR OUT
          TFR  D,X            (X = FIELD WIDTH : LAST ZONE)
          PULS B
          LDA  #255           A FILE HAS NO LINE WRAP (AND A NONZERO WIDTH)
          BRA  LA37C
...
; basic309: was "BRA LAC7C" (reenter BASIC's input loop) unconditionally.
; Redirected through LINEDONE_VEC (defaults to LAC7C, same behavior as
; before) so LOAD can retarget it to its own loop while feeding lines in
; from a file one at a time through this exact routine -- the same path
; typing a numbered line at the prompt already takes -- then restore it
; before handing back to the real prompt. See LOAD below.
          JMP  [LINEDONE_VEC] 
...
LAD19     JSR  CLOSEALL       basic309: NEW / LOAD CLOSE ANY OPEN DATA FILES
...
*
; basic309: LOAD/SAVE. Both end like LIST does (JMP LAC73, not RTS) --
; loading/saving replaces or reflects the *whole* program, so whatever
; might follow on the same direct-mode line wouldn't mean anything once
; either finishes, same reasoning LIST already applies to itself.
;------------------------------------------------------------------------------
; LOAD "name": parses the filename (reusing the same expression evaluator
; and TM-error-checked string extraction every string-valued expression
; already goes through), clears the current program (LAD19, same as NEW),
; then reads the file one line at a time and feeds each one through
; LACA5 -- the exact routine that fires when a numbered line is typed at
; the OK prompt. LACA5 itself never returns to its caller (it always
; ends by jumping onward); LINEDONE_VEC (see LACE9's tail) is
; retargeted here so that "onward" is this loop instead of the real
; prompt, then restored before the final return.
;------------------------------------------------------------------------------
LOAD      JSR  LB156          EVALUATE THE FILENAME EXPRESSION
          JSR  LB654          �TM� ERROR IF NOT A STRING; B=LEN, X=ADDR
          JSR  FMTNAME        -> FNBUF (11-BYTE 8.3 NAME)
          LDX  #FNBUF
          LDA  #FOPEN_READ
          TFR  A,E
          LDA  #B_FOPEN_NAME
          ...
          LDX  #LOAD_CONT
          STX  LINEDONE_VEC   REDIRECT LACA5'S COMPLETION TO US
LOAD_CONT LDB  FILEREF
          LDX  #LINBUF+1
          LDY  #LBUFMX
          LDA  #B_READLINE
          SWI2
          ...
          CMPX #0
          BEQ  LOAD_CLOSE     EOF
          TFR  X,D            X (accumulator offsets need A/B/D, not X)
          LDY  #LINBUF+1
          LEAY D,Y            Y = LINBUF+1 + ACTUAL LENGTH JUST READ
          CLR  ,Y             NULL-TERMINATE, SAME AS A TYPED LINE
          LDX  #LINBUF        GETNCH PRE-INCREMENTS, SO POINT ONE BEFORE
          STX  CHARAD         THE REAL TEXT (SAME AS LA390'S OWN RETURN)
          JSR  GETNCH         SAME SEQUENCE THE OK-PROMPT PATH USES
          JMP  LACA5          PARSE/CRUNCH/INSERT; LANDS BACK AT LOAD_CONT
LOAD_CLOSE
          LDB  FILEREF
          LDA  #B_FCLOSE_NAME
          SWI2
LOAD_DONE LDX  #LAC7C
          STX  LINEDONE_VEC   RESTORE NORMAL INTERACTIVE BEHAVIOR
          JMP  LAC73          BACK TO THE OK PROMPT
LOAD_NF   LDB  #26*2          "NE" FILE NOT FOUND (SEE L890D IN THE ERROR
          JMP  LAC46          TABLE) -- SAME CONVENTION AS UL/OM/ETC ABOVE
;------------------------------------------------------------------------------
; SAVE "name": walks the program exactly like LIST does (TXTTAB onward,
; following each line's "next line" pointer), but instead of sending the
; line number + detokenized text to the console, builds "<number> <text>"
; in SAVELINE and writes it as one line via B_WRITELINE (which appends the
; line's terminating CR itself).
;------------------------------------------------------------------------------
SAVE      JSR  LB156
          JSR  LB654
          JSR  FMTNAME
...
SAVE_LOOP LDD  ,X             ADDRESS OF NEXT LINE (0 = END OF PROGRAM)
          BEQ  SAVE_CLOSE
          STX  SAVECUR
          LDD  2,X            THIS LINE'S NUMBER
          LDY  #SAVELINE
          JSR  FMTNUM         -> SAVELINE, Y = PTR PAST THE DIGITS
          LDA  #SPACE
          STA  ,Y+
          STY  SAVEPOS
          LDX  SAVECUR
          JSR  LB7C2          UNCRUNCH INTO LINBUF+1 (NULL-TERMINATED)
          LDX  #LINBUF+1
          LDY  SAVEPOS
SAVE_APP  LDA  ,X+
          BEQ  SAVE_APPD
          ...
          BRA  SAVE_APP
SAVE_APPD TFR  Y,D
          SUBD #SAVELINE      D = TOTAL LINE LENGTH
          TFR  D,Y
          LDX  #SAVELINE
          LDB  FILEREF
          LDA  #B_WRITELINE
          SWI2
          BCS  SAVE_WRITEERR  WRITE FAILED MID-SAVE -- CLOSE, THEN ERROR
          LDX  SAVECUR
          LDX  ,X             NEXT LINE'S ADDRESS
          BRA  SAVE_LOOP
SAVE_CLOSE
          LDB  FILEREF
          LDA  #B_FCLOSE_NAME
...
PFN_NAME  LDA  ,X
          ...
          BNE  PFN_NAME
PFN_NAMEDONE
          LDX  #DIRBUF+8      FORCE X TO THE EXTENSION FIELD REGARDLESS
          LDA  ,X             OF HOW THE NAME LOOP ABOVE ENDED
          CMPA #SPACE
          BEQ  PFN_DONE       BLANK EXTENSION -- NOTHING MORE TO PRINT
          LDA  #'.
          JSR  PUTCHR
          LDB  #3
PFN_EXT   LDA  ,X
...
;
; BASIC file numbers 1..NFILES are opened with OPEN and named in PRINT#, INPUT#,
; LINE INPUT#, WRITE#, CLOSE and EOF()/LOF()/LOC(). Each maps to a DOS fileref
; (B_FOPEN_NAME etc., dos/dos.asm) via the FT_* tables. PRINT#/INPUT# work by
; setting DEVNUM = the file number for the duration of the statement: PUTCHR then
; writes to the file instead of the console, and the INPUT item-fetch reads from
; it. Any error (LAC46) clears DEVNUM again.
;==============================================================================
;------------------------------------------------------------------------------
; IN: B = a file number. OUT: X -> FT_MODE[B] (FT_REF is 5 bytes on, FT_COL 10).
; "DN" (bad file number) unless 1..NFILES.
;------------------------------------------------------------------------------
CHKFN     TSTB
          BEQ  DN_ERR
          CMPB #NFILES
...
DN_ERR    LDB  #19*2
          JMP  LAC46
FM_ERR    LDB  #21*2          "FM" BAD FILE MODE
          JMP  LAC46
NO_ERR    LDB  #22*2          "NO" FILE NOT OPEN
          JMP  LAC46
SYN_ERR   JMP  LB277
;------------------------------------------------------------------------------
; IN: B = a file number that must be open for output (or append). OUT: B kept,
; X -> FT_MODE[B]. "NO" if not open, "FM" if it's open some other way.
;------------------------------------------------------------------------------
NEEDOUT   BSR  CHKFN
          LDA  ,X
          BEQ  NO_ERR
          ...
          BNE  FM_ERR
NEED_OK   RTS
; The same for a file that must be open for input.
NEEDIN    BSR  CHKFN
          LDA  ,X
          BEQ  NO_ERR
          CMPA #FM_IN
          ...
          RTS
;------------------------------------------------------------------------------
; Parses the file number after PRINT/INPUT/WRITE/CLOSE/...: "#n" or a bare
; expression n. IN: A = the current character. OUT: B = n (0-255), A = the
; character after it.
;------------------------------------------------------------------------------
FILENUM   CMPA #'#
          BNE  FILENUM_X
          JMP  LB709          SKIP THE '#', EVALUATE
FILENUM_X JMP  LB70B
;------------------------------------------------------------------------------
; After a file number: consumes the comma that follows it. OUT: A = the next
; character, Z set if that is the end of the statement. Anything other than a
; comma or the end of the statement is a syntax error.
;------------------------------------------------------------------------------
SKIPCOMMA JSR  GETCCH
          BEQ  SC_RET
          CMPA #',
          BNE  SYN_ERR
          JMP  GETNCH
SC_RET    RTS
;------------------------------------------------------------------------------
; IN: A = a DOS error code (carry was set). Raises the matching BASIC error.
;------------------------------------------------------------------------------
DOSERR    LDX  #DOSERRTAB
DE_LOOP   TST  ,X
          BEQ  DE_DEFAULT
          CMPA ,X++
          BNE  DE_LOOP
...
DE_DEFAULT LDB 1,X
          JMP  LAC46
DOSERRTAB FCB  ERR_NOTFOUND,26*2      "NE" FILE NOT FOUND
          FCB  ERR_NOSPACE,28*2       "DF" DISK FULL
          FCB  ERR_ISOPEN,18*2        "AO" FILE ALREADY OPEN
...
;------------------------------------------------------------------------------
; OPEN "mode",[#]n,"file"  or  OPEN "file" FOR INPUT|OUTPUT|APPEND AS [#]n.
; Mode letters: I (input), O (output -- creates or empties the file), A
; (append -- creates or adds to the file). Unlike SAVE/LOAD, a name typed with no
; extension gets none (see OPENNAME). FOR/AS: "AS" isn't a token, and INPUT is
; the only mode word that is one -- OUTPUT and APPEND stay literal letters
; (crunch leaves a word alone once its first letter starts no keyword).
;------------------------------------------------------------------------------
OPEN      LDD  #128           DEFAULT RECORD LENGTH (RANDOM FILES)
          STD  OPLEN
          JSR  LB156          EVALUATE THE FIRST STRING
          JSR  LB654          B = LEN, X = ADDR (TM ERROR IF NUMERIC)
          PSHS X,B
          JSR  GETCCH
          CMPA #',
          PULS B,X
          BEQ  OPEN_OLD       "mode",#n,"file"
          JSR  OPENNAME       X,B = THE FILE NAME -> FNBUF
          JSR  GETCCH
          CMPA #TOK_FOR
          BNE  OPEN_NOFOR     NO "FOR": RANDOM ACCESS
          JSR  GETNCH
          LDX  CHARAD         X -> THE FIRST CHARACTER OF THE MODE WORD
          LDB  #FM_OUT
          CMPA #'O
          BEQ  OPEN_SKIPWORD
          LDB  #FM_APP
          ...
          CMPA #TOK_INPUT
          LBNE SYN_ERR
          LEAX 1,X            A ONE-BYTE TOKEN: NOTHING MORE TO SKIP
          BRA  OPEN_GOTWORD
OPEN_SKIPWORD
          LEAX 1,X
          LDA  ,X
...
OPEN_GOTWORD
          STB  OPMODE
          LEAX -1,X
          STX  CHARAD         GETNCH READS AT X, PAST ANY SPACES
          JSR  GETNCH
OPEN_A    CMPA #'A
          LBNE SYN_ERR
          JSR  GETNCH
          ...
          JSR  FILENUM
          STB  OPNUM
          JSR  GETCCH         OPTIONAL  LEN=record length
          BEQ  OPEN_GO
          CMPA #$FF
          LBNE SYN_ERR
          JSR  GETNCH
          ...
          BSR  OPEN_LENX
          BRA  OPEN_GO
OPEN_LENX JSR  LB73D          THE RECORD LENGTH (CHECKED WHEN A RANDOM FILE IS OPENED)
          STX  OPLEN
          RTS
OPEN_NOFOR LDB #FM_RND        NO "FOR": RANDOM ACCESS
          STB  OPMODE
          BRA  OPEN_A         (A = THE CURRENT CHARACTER: "AS" MUST FOLLOW)
OPEN_OLD  TSTB
          LBEQ FM_ERR
          LDA  ,X
          ANDA #$DF           FOLD LOWER CASE
          LDB  #FM_IN
          CMPA #'I
          BEQ  OO_GOT
          LDB  #FM_OUT
          ...
          LBNE FM_ERR
OO_GOT    STB  OPMODE
          JSR  GETNCH         PAST THE COMMA
          JSR  FILENUM
          STB  OPNUM
          JSR  LB26D          THE COMMA AFTER THE FILE NUMBER
          JSR  LB156
          JSR  LB654
          JSR  OPENNAME
          JSR  GETCCH         OPTIONAL  ,record length
          CMPA #',
          BNE  OPEN_GO
          JSR  GETNCH
          BSR  OPEN_LENX
OPEN_GO   LDB  OPNUM
          JSR  CHKFN
          TST  ,X
          BNE  AO_ERR         ALREADY OPEN AS THAT NUMBER
          LDA  OPMODE
          CMPA #FM_RND
          BNE  OG_DOS
          LDD  OPLEN          RECORD LENGTH MUST BE 1-256
          LBEQ LB44A
          CMPD #256
          LBHI LB44A
OG_DOS    LDA  OPMODE
          DECA                DOS MODE = FM_ CODE - 1
          TFR  A,E
          LDX  #FNBUF
          LDA  #B_FOPEN_NAME
          SWI2
          ...
          CMPA #FM_RND
          BNE  OG_RET
          LDB  OPNUM          A RANDOM FILE: RECORD LENGTH, RECORD 0, A CLEAR BUFFER
          ASLB
          LDX  #RECLEN
          ABX
          LDD  OPLEN
...
CLOSEONE  JSR  CHKFN
          LDA  ,X
          BEQ  CO_RET
          CLR  ,X             THE NUMBER IS FREE EVEN IF THE DOS CLOSE FAILS
          LDB  5,X
          LDA  #B_FCLOSE_NAME
          SWI2
          BCS  CO_FAIL
CO_RET    RTS
CO_FAIL   JMP  DOSERR
; Closes every open file. Trashes A, B, X.
CLOSEALL  LDB  #NFILES
CA_LOOP   PSHS B
          BSR  CLOSEONE
          PULS B
          ...
          RTS
;------------------------------------------------------------------------------
; PRINT #n, ... -- PRINT with its output sent to file n; also PRINT #n, USING.
;------------------------------------------------------------------------------
PRINT_FILE
          JSR  FILENUM
          JSR  NEEDOUT
          ...
          JSR  SKIPCOMMA
          BEQ  PRF_NL
          JSR  LB8FE          PRINT'S ITEM LOOP (AND PRINT USING)
          BRA  PRF_DONE
PRF_NL    LDA  #CR
          JSR  PUTCHR
PRF_DONE  CLR  DEVNUM
          RTS
;------------------------------------------------------------------------------
; PUTCHR for a file (DEVNUM <> 0): CR is written as CR LF and resets the file's
; print column; everything else is written as is. Preserves A, B, X.
;------------------------------------------------------------------------------
PUTCHR_FILE
          PSHS X,B,A
          LDB  DEVNUM
          ...
          BRA  PCF_LAST
PCF_PLAIN INC  10,X
PCF_LAST  BSR  PCF_BYTE
          PULS A,B,X,PC
; IN: A = the byte, X -> FT_MODE[DEVNUM].
PCF_BYTE  LDB  5,X
          TFR  A,E
          LDA  #B_FPUTC
          SWI2
...
PCF_ERR   JMP  DOSERR
;------------------------------------------------------------------------------
; WRITE [#n,] item, item, ... -- like PRINT, but strings are written in quotes,
; numbers without padding, items separated by commas, and it ends with a
; newline: the format INPUT # reads back exactly. Without #n it goes to the
; console.
;------------------------------------------------------------------------------
WRITE     CMPA #'#
          BEQ  WR_FILE
...
WR_NL     LDA  #CR
          JMP  PUTCHR
;------------------------------------------------------------------------------
; INPUT #n, var, var, ... -- the item fetch (FILEITEM) below is hooked into
; INPUT's own per-variable loop at LB04E.
;------------------------------------------------------------------------------
INPUT_FILE
          JSR  FILENUM
          JSR  NEEDIN
          STB  DEVNUM
          JSR  SKIPCOMMA
          LBEQ SYN_ERR        INPUT #n NEEDS VARIABLES
          LDX  #LINBUF
          CLR  ,X             EMPTY BUFFER: THE FIRST ITEM IS READ FROM THE FILE
          JSR  LB049
          CLR  DEVNUM
          RTS
;------------------------------------------------------------------------------
; LINE INPUT #n, A$ -- reads the rest of the current line, up to a CR, LF or
; CR LF (or 249 characters), into A$.
;------------------------------------------------------------------------------
LINEINPUT_FILE
          JSR  FILENUM
          JSR  NEEDIN
          ...
          RTS
;------------------------------------------------------------------------------
; OUT: X = the current file's DOS fileref in B, FI_REF set. Uses DEVNUM.
;------------------------------------------------------------------------------
FILEREF_SET
          LDB  DEVNUM
          LDX  #FT_MODE
...
FILELINE  BSR  FILEREF_SET
          ...
          BHI  FL_READ
          LDB  #23*2          AT THE END: "IE" INPUT PAST END
          JMP  LAC46
FL_READ   LDB  FI_REF
          LDX  #LINBUF+1
          LDY  #LBUFMX-1
...
FL_ERR    JMP  DOSERR
;------------------------------------------------------------------------------
; Fetches the next INPUT # item from the file into LINBUF+1 (NUL-terminated) with a
; comma in LINBUF, and returns X = LINBUF, ready for INPUT's own parser. Leading
; spaces and line breaks are skipped; an item is either "quoted" (kept with its
; quotes -- INPUT strips them) or runs up to a comma, CR or LF. The terminator is
...
FI_CR     JSR  FI_GETC        A CR TAKES A FOLLOWING LF WITH IT
          ...
          PULS X
          BRA  FI_DONE
FI_IE     LDB  #23*2          "IE" INPUT PAST END
          JMP  LAC46
FI_STORE  CMPX #LINBUF+LBUFMX-1
          BHS  FIS_RET
          STA  ,X+
FIS_RET   RTS
; OUT: A = the next byte, carry set at end of file. Preserves B, X.
FI_GETC   PSHS B
          LDB  FI_REF
          LDA  #B_FGETC
          SWI2
          PULS B,PC
;------------------------------------------------------------------------------
; EOF(n), LOF(n), LOC(n): the argument is in FPA0. FILEFN -> B = the (open) file
; number, X = its size, Y = its position.
;------------------------------------------------------------------------------
FILEFN    JSR  LB3E9          FPA0 -> D (FC ERROR IF NEGATIVE)
          TSTA
          LBNE LB44A
          JSR  CHKFN
          STB  OPNUM
...
EOFFN     BSR  FILEFN
          LDA  OPMODE
          CMPA #FM_RND        RANDOM FILE: TRUE IF THE LAST GET RAN PAST THE END
          BNE  EOF_SEQ
          LDB  OPNUM
          LDX  #FT_MODE
          ABX
...
EOF_FALSE CLRA
          CLRB
          JMP  GIVABF
; LOF(n): the file's length in bytes (0-65535).
LOFFN     BSR  FILEFN
          TFR  X,D
GIVABU    PSHS A              D AS AN UNSIGNED NUMBER: GIVABF TREATS IT AS SIGNED,
          JSR  GIVABF         SO ADD 65536 BACK WHEN BIT 15 WAS SET
          PULS A
          TSTA
          BPL  GU_RET
          LDX  #FP65536
          JMP  LB9C2          ADD (X) TO FPA0
GU_RET    RTS
FP65536   FCB  $91,$00,$00,$00,$00
; LOC(n): for a sequential file, the number of 128-byte records read or written
; so far (rounded up).
LOCFN     BSR  FILEFN
          LDA  OPMODE
          CMPA #FM_RND        RANDOM FILE: THE LAST RECORD GOT OR PUT
          BNE  LOC_SEQ
          LDB  OPNUM
          ASLB
          LDX  #RECNO
          ...
          JMP  GIVABF
LOC_SEQ   TFR  Y,D
          PSHS B              THE POSITION'S LOW BYTE
          ASLB
          ROLA                A = BITS 14-7 OF THE POSITION, CARRY = BIT 15
          TFR  A,B
          LDA  #0
          ROLA                D = POSITION / 128
          PSHS D
          LDA  2,S
          ANDA #$7F           ANY REMAINDER?
          PULS D
          LEAS 1,S
          BEQ  LOC_EXACT
          ADDD #1             ROUND UP
LOC_EXACT JMP  GIVABF

;==============================================================================
; Random-access files. OPEN "R",#n,"file"[,reclen] (or OPEN "file" AS #n [LEN=reclen])
; gives file n a 256-byte record buffer and a record length of 1-256 (default 128).
; FIELD carves the buffer into string variables (their descriptors point straight
; into it); GET/PUT move a whole record between the file and the buffer; LSET/RSET
; put text into a field; MKI$/MKS$ and CVI/CVS turn numbers into 2- and 4-byte
; strings and back. Record numbers run 1..32767; the file stays under 64KB.
;==============================================================================
; IN: B = a file number that must be open as a random file. OUT: B kept, X -> FT_MODE[B].
NEEDRND   JSR  CHKFN
          LDA  ,X
          LBEQ NO_ERR
          CMPA #FM_RND
          LBNE FM_ERR
          RTS
; Zeroes file OPNUM's record buffer. Trashes A, B, X, Y, W.
ZEROBUF   LDA  OPNUM
          DECA
          CLRB
          ADDD #RBUF
          ...
          RTS
;------------------------------------------------------------------------------
; FIELD [#]n, width AS var$ [, width AS var$ ...] -- each variable becomes a
; window onto the next `width` bytes of the record buffer.
;------------------------------------------------------------------------------
FIELD     JSR  FILENUM
          JSR  NEEDRND
          STB  OPNUM
          ...
          JSR  SKIPCOMMA
          BEQ  FLD_RET
FLD_LOOP  JSR  LB70B          THE WIDTH (0-255)
          STB  FLDW
          CMPA #'A
          LBNE SYN_ERR
          JSR  GETNCH
          ...
          LBNE SYN_ERR
          JSR  GETNCH
          JSR  LB357          X -> THE VARIABLE'S DESCRIPTOR
          JSR  LB146          "TM" UNLESS A STRING VARIABLE
          STX  FLDP
          LDB  FLDW
          CLRA
          ADDD RTMP           WHERE THIS FIELD WOULD END
          PSHS D
          LDB  OPNUM
          ASLB
          LDX  #RECLEN
          ...
          LDD  ,X
          CMPD ,S++
          LBLO LB44A          BEYOND THE RECORD: "FC"
          LDA  OPNUM          THIS FIELD'S ADDRESS = BUFFER + OFFSET
          DECA
          CLRB
          ADDD #RBUF
          ADDD RTMP
...
FLD_RET   RTS
;------------------------------------------------------------------------------
; LSET var$ = expr / RSET var$ = expr -- copy text into a FIELD variable, left or
; right justified in its width (space padded, truncated on the right if too long).
; Only fielded variables: a descriptor not pointing into a record buffer is "FC".
;------------------------------------------------------------------------------
LSET      CLR  RJUST
          BRA  LR_GO
RSET      LDA  #1
...
LR_GO     JSR  LB357
          ...
          JSR  LB26F
          JSR  LB156
          JSR  LB654          B = SOURCE LENGTH, X = ITS ADDRESS
          LDY  FLDP
          LDA  ,Y             THE FIELD'S WIDTH
          LDU  2,Y            ITS ADDRESS
          CMPU #RBUF
          LBLO LB44A
          CMPU #RBUF+NFILES*256
          LBHS LB44A
          STA  FLDW
          CMPB FLDW
          BLS  LR_OK
          LDB  FLDW           LONGER THAN THE FIELD: TRUNCATE
LR_OK     STB  FLDL
          LDA  FLDW
          SUBA FLDL
          STA  FLDPAD
          ...
          BRA  LR_PAD
LR_RIGHT  BSR  LR_PAD
LR_MOVE   LDB  FLDL           COPY FLDL BYTES FROM X TO U
          BEQ  LRM_R
LRM_L     LDA  ,X+
          STA  ,U+
          DECB
          BNE  LRM_L
LRM_R     RTS
LR_PAD    LDB  FLDPAD         STORE FLDPAD SPACES AT U
          BEQ  LRP_R
          LDA  #SPACE
LRP_L     STA  ,U+
          DECB
...
LRP_R     RTS
;------------------------------------------------------------------------------
; GET [#]n [,record] / PUT [#]n [,record] -- read or write a whole record. With no
; record number, the one after the last used. GET past the end of the file gives a
; zero-filled record and EOF(n) true (no error); PUT past the end extends the file
; (the gap reads as zeros).
;------------------------------------------------------------------------------
GET       CLR  RWFLG          (A MUST SURVIVE: IT IS THE CURRENT CHARACTER, MAYBE "#")
          BRA  GP_GO
PUT       LDB  #1
          STB  RWFLG
GP_GO     JSR  FILENUM
...
GP_HAVE   CMPX #0
          ...
          ASLB
          LDY  #RECNO
          STX  B,Y            THIS IS NOW THE CURRENT RECORD (LOC)
          LEAX -1,X
          LDU  #RECLEN
          LEAU B,U            U -> THIS FILE'S RECORD LENGTH
          TFR  X,D
          MULD ,U             Q = (RECORD-1) * LENGTH
          CMPD #0
          LBNE LB44A          OVER 64KB
          TFR  W,X            X = THE RECORD'S BYTE OFFSET
          TFR  X,D
          ADDD ,U
          LBCS LB44A          ITS END WOULD BE OVER 64KB
          PSHS X
          LDB  OPNUM
          LDX  #FT_MODE
          ABX
          ...
          TST  RWFLG
          BNE  GP_PUT
          JSR  ZEROBUF        SO A SHORT READ LEAVES ZEROS
          LDA  OPNUM
          DECA
          CLRB
          ADDD #RBUF
...
GET_FLAG  PSHS A
          ...
          ABX
          PULS A
          STA  10,X           FT_COL = "LAST GET WAS SHORT" = EOF(n)
          RTS
GP_PUT    LDA  OPNUM
          DECA
          CLRB
          ...
          RTS
;------------------------------------------------------------------------------
; CVI(s$): the 2-byte string MKI$ makes -> the integer. CVS(s$): 4 bytes (exponent,
; then the top 3 mantissa bytes with the sign in the first) -> the number.
; MKI$(n) / MKS$(x): the reverse. MKS$ keeps 24 bits of mantissa, so a value
; read back can differ from the original in about the 8th digit.
;------------------------------------------------------------------------------
CVIFN     JSR  LB654
          CMPB #2
          LBLO LB44A
...
CVSFN     JSR  LB654
          ...
          LDX  #V40
          JMP  LBC14
MKIFN     JSR  INTCNV         D = THE INTEGER ("FC" IF OUT OF RANGE)
          PSHS D
          LDB  #2
          JSR  LB56D          RESERVE 2 BYTES OF STRING SPACE
          JSR  LB511
          PULS D
          STD  ,X
          JMP  LB69B          (STRING RESULT: SKIP THE CALLER'S TYPE CHECK)
MKSFN     JSR  LB143          "TM" IF A STRING
          LDB  #4
          JSR  LB56D
          JSR  LB511
          PSHS X
          LDX  #V40
          JSR  LBC35          PACK FPA0 INTO V40..V44
          PULS X
          LDD  V40
          STD  ,X
          LDD  V42
...
; terminated; Y = pointer to the terminating null, so a caller can append
; more text right after. Uses compare/subtract against descending powers
; of ten rather than a divide instruction, since the 6309's DIVD only
; gives an 8-bit quotient -- too narrow for a line number over 255.
; Trashes A, B, D, X.
;------------------------------------------------------------------------------
FMTNUM    STD  FMTVAL
          CLR  FMTLZ           0 = STILL SUPPRESSING LEADING ZEROS
          LDX  #FMTPOW10
FN_PLOOP  LDD  ,X
          BEQ  FN_ONES         A ZERO TABLE ENTRY MARKS THE ONES PLACE
          STD  FMTPOW
          CLR  FMTDIG
FN_DLOOP  LDD  FMTVAL
          CMPD FMTPOW
...
FN_DDONE  LDA  FMTDIG
          BEQ  FN_MAYBEZ
          LDB  #1
          STB  FMTLZ           A NONZERO DIGIT -- STOP SUPPRESSING
          BRA  FN_EMIT
FN_MAYBEZ TST  FMTLZ
          BEQ  FN_SKIP         STILL ALL ZEROS SO FAR -- SUPPRESS THIS ONE
FN_EMIT   ADDA #'0
          STA  ,Y+
FN_SKIP   LEAX 2,X
          BRA  FN_PLOOP
FN_ONES   LDA  FMTVAL+1        REMAINING VALUE (0-9) IS THE ONES DIGIT
          ADDA #'0
          STA  ,Y+
          CLR  ,Y
          RTS
FMTPOW10  FDB  10000,1000,100,10,0
*
* FOR
*                              
...
*                              
* BREAK CHECK                      
LADEB     JSR  KEYIN          GET A KEYSTROKE ENTRY 
...
LAE22
          ...
          BMI  LAE22_STOP     STOP (NEGATIVE): FILES STAY OPEN SO CONT WORKS
          LDX  CURLIN
          LEAX 1,X
          BEQ  LAE22_END      DIRECT MODE ($FFFF): EVERY TYPED LINE ENDS HERE -- KEEP THE FILES
          JSR  CLOSEALL       basic309: A PROGRAM'S END / RUNNING OFF ITS END CLOSES ALL FILES
LAE22_END JMP  LAC73          BRANCH TO MAIN LOOP OF BASIC IF END
LAE22_STOP LDX #LABF2-1       POINT TO CR, "BREAK" MESSAGE
...
LAE6F     JSR  CLOSEALL       basic309: CLEAR CLOSES ALL DATA FILES
          JMP  LAD26          ERASE ALL VARIABLES, INITIALIZE POINTERS, ETC
LAE72     JMP  LAC44          �OM� ERROR 
*                              
* RUN                          
RUN       JSR  CLOSEALL       basic309: RUN CLOSES ALL DATA FILES
          JSR  GETCCH         * GET CURRENT INPUT CHARACTER
...
LAFD6     TST  DEVNUM         basic309: bad INPUT/READ data. Restored from Color BASIC (this
          BEQ  LAFDF          port had dropped the test, so the error code was garbage):
          LDB  #17*2          from a file it's "FD" (BAD FILE DATA); from the keyboard,
LAFDC     JMP  LAC46          "?REDO" (LAFDF); JMP TO ERROR HANDLER
...
*                              
* INPUT                        
INPUT     CMPA #'#            basic309: INPUT #n, ... reads from a file
          LBEQ INPUT_FILE
          LDB  #11*2          �ID� ERROR
...
* NOT COMING FROM SCREEN
          TST  DEVNUM         basic309: INPUT # -- THE NEXT ITEM COMES FROM THE FILE
          BEQ  LB04E_CON
          JSR  FILEITEM       (RETURNS X = LINBUF, THE ITEM PREFIXED WITH A COMMA)
          BRA  LB069
LB04E_CON JSR  LB9AF          SEND A '?' TO CONSOLE OUT
...
* READ/INPUT A STRING VARIABLE                      
          ...
          TST  DEVNUM
          BEQ  LB08A
          CLRA                basic309: a file item is one whole field -- a colon inside it is data
LB08A     STA  CHARAC         SAVE AS TERMINATOR I
          LDA  #',            COMMA
...
* PRINT                        
PRINT     CMPA #'#            basic309: PRINT #n, ... writes to a file
          LBEQ PRINT_FILE
          JSR  GETCCH         (THE COMPARE ABOVE DISTURBED THE END-OF-LINE FLAG)
          BEQ  LB958          BRANCH IF NO ARGUMENT
...
L89C0     JSR  GETNCH         GET A CHAR FROM BASIC (PAST THE INPUT TOKEN)
          CMPA #'#            basic309: LINE INPUT #n, A$ reads a line from a file
          LBEQ LINEINPUT_FILE
          JSR  L8866          �BS� ERROR IF IN DIRECT MODE (LEAVES A ALONE)
...
LINE_STORE JSR LB357          SEARCH FOR A VARIABLE (LINE INPUT # JOINS HERE, LINBUF FILLED)
