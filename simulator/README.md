...
+ `SHELL.COM` + `EDIT.COM` + `PUGASM.COM` + `PUGLINK.COM` + `BASIC.COM`, the `$C000-$EFFF` image extracted from `exbasrom309.s19`
...
- `test_pugasm.cpp` -- PUGASM.COM and PUGLINK.COM (`../pugasm/`) on the emulated machine. The
  lwtools build is the reference, byte for byte and listing for listing:
  - PUGASM must assemble the shell, the editor, DOS, BASIC (as S-records), itself, PUGLINK, and
    the BIOS's modules (as object files).
  - PUGLINK must link the BIOS to lwlink's S-records and map, and the whole BIOS must build on
    the machine.
  - The copies they make must work, the programs they make must run, and errors must leave no
    files.
  - With lwtools present, they must also match lwasm and lwlink on `test_asm/pugasm`: every
